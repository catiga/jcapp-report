package com.jeancoder.report.entry.members_buy
...
import com.jeancoder.report.ready.dto.members_buy.MembersBuyPage
...
def order_status=OrderConstants._order_status_taked_+","+OrderConstants._order_status_payed_pub_failed_;
...
//	def list = RemoteUtil.connect('trade', '/incall/allow_pt', null);
	...
	MembersBuyPage page = NativeUtil.connect(MembersBuyPage.class, "crm", "order/member_order_list", ["pn":pn,"ps":ps,"pid":pid,"start_data":start_data,"end_data":end_data,"order_status":order_status])
	if (!page.available) {
		page.pn = 1;
		page.ps = 10;
	...
	MembersBuyPage page_csv = NativeUtil.connect(MembersBuyPage.class, "crm", "order/member_order_list", ["pn":"1","ps":"10000","pid":pid,"start_data":start_data,"end_data":end_data,"order_status":order_status])
	...
	page.totalCount = 0;
	result.addObject("page",page);
	result.addObject("error_msg","查询会员购买报表失败");
}
...
return result.setView("members_buy/index");