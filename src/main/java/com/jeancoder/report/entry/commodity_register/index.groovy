package com.jeancoder.report.entry.commodity_register
...
//	def list = RemoteUtil.connect('trade', '/incall/allow_pt', null);
	...
	GoodsSalePage page = NativeUtil.connect(GoodsSalePage, "scm", "order/order_item_list", ["pn":pn,"ps":ps,"pid":pid,"start_data":start_data,"end_data":end_data,"oss":oss,"flag":"1"])
	if (!page.available) {
		page.pn = 1;
		page.ps = 10;
	...
	GoodsSalePage page_csv = NativeUtil.connect(GoodsSalePage, "scm", "order/order_item_list", ["pn":"1","ps":"10000","pid":pid,"start_data":start_data,"end_data":end_data,"oss":oss,"flag":"1"])
	...
	page.totalCount = 0;
	result.addObject("page",page);
	result.addObject("error_msg","查询商品收银台报表失败");
}
...
return result.setView("commodity_register/index");