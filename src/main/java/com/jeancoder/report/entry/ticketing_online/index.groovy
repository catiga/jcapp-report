package com.jeancoder.report.entry.ticketing_online
...
//	def list = RemoteUtil.connect('trade', '/incall/allow_pt', null);
	...
	TicketSalePage page = NativeUtil.connect(TicketSalePage, "ticketingsys", "ticketing/ticketing_online", ["pn":pn,"ps":ps,"pid":pid,"start_data":start_data,"end_data":end_data,"order_status":order_status])
	if (!page.available) {
		page.pn = 1;
		page.ps = 10;
	...
	TicketSalePage page_csv = NativeUtil.connect(TicketSalePage, "ticketingsys", "ticketing/ticketing_online", ["pn":"1","ps":"10000","pid":pid,"start_data":start_data,"end_data":end_data,"order_status":order_status])
	...
	page.totalCount = 0;
	result.addObject("page",page);
	result.addObject("error_msg","查询票务网售报表失败");
}
...
return result.setView("ticketing_online/index");