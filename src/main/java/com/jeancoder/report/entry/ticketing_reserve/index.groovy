package com.jeancoder.report.entry.ticketing_reserve
...
import com.jeancoder.report.ready.dto.ticket.TicketReservePage
...
//	def list = RemoteUtil.connect('trade', '/incall/allow_pt', null);
	...
	TicketReservePage page = NativeUtil.connect(TicketReservePage, "ticketingsys", "ticketing/ticketing_reserve", ["pn":pn,"ps":ps,"pid":pid,"start_data":start_data,"end_data":end_data,"order_status":order_status])
	if (!page.available) {
		page.pn = 1;
		page.ps = 10;
	...
	TicketReservePage page_csv = NativeUtil.connect(TicketReservePage, "ticketingsys", "ticketing/ticketing_reserve", ["pn":"1","ps":"10000","pid":pid,"start_data":start_data,"end_data":end_data,"order_status":order_status])
	...
	page.totalCount = 0;
	result.addObject("page",page);
	result.addObject("error_msg","查询票务预订报表失败");
}
...
return result.setView("ticketing_reserve/index");